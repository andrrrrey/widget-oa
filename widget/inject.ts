...
(window as WindowWithQueue)._chatWidgetQueue = (window as WindowWithQueue)._chatWidgetQueue || [];

// Create temporary function to queue calls
if (!(window as WindowWithQueue).initWidgetPlatform) {
  (window as WindowWithQueue).initWidgetPlatform = (config: ChatWidgetConfig) => {
...
function initWidgetPlatform(config: ChatWidgetConfig) {
  ...
  button.innerHTML = `
    ${finalConfig.svgIcon ? finalConfig.svgIcon : `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
function toggleChat(config: ChatWidgetConfig) {
  ...
  const button = document.getElementById('openai-chat-widget-button') as HTMLButtonElement;
